# TCF Canada — 2-Day Review Plan

## Overview
- 3 hours/day, 3 sessions of 1 hour each
- Write everything on **paper with pen**
- Goal: make what you already know **automatic**

---

# DAY 1

## Session 1: Review & Memorize (1 hour)

### Read error files (20 min)
- [ ] Read `french-grammar-weak-points.md` — focus on points 5, 15, 17, 18, 27
- [ ] Read `french-prepositions-common-errors.md` — focus on: recommander de, se concentrer sur, détails sur, curiosité pour

### Memorize autopilot sentences — write by hand 3x each (20 min)
- [ ] T1 informal opening: *Salut Mathieu, j'espère que tu vas bien ! Je t'écris pour...*
- [ ] T1 formal opening: *Bonjour Madame Dupont, je me permets de vous écrire au sujet de...*
- [ ] T2 Frame A opening: *Je tenais à vous raconter [X]. Ce fut une expérience enrichissante et inoubliable.*
- [ ] T2 time markers: *Dans un premier temps... Par la suite...*
- [ ] T2 highlight: *Ce qui m'a le plus marqué, c'est...*
- [ ] T2 reflection: *Cette expérience m'a rappelé à quel point...*
- [ ] T2 Frame A closing: *J'ose espérer que l'avenir nous réserve encore de tels instants.*
- [ ] T2 Frame D opening: *Personnellement, je suis convaincu(e) que [position].*
- [ ] T2 Frame D argument: *Mon expérience personnelle m'a appris que...*
- [ ] T2 Frame D nuance: *Certes, [concession], mais [counter].*
- [ ] T2 Frame E transition: *Personnellement, j'ai trouvé que cette expérience était...*
- [ ] T3 P1: *Le sujet sur [X] oppose deux visions divergentes. D'un côté... De l'autre...*
- [ ] T3 P2: *Personnellement, si [adj] que soient les [nom], on ne saurait ignorer... En effet... De surcroît...*
- [ ] T3 P3: *Certes, [X] est/sont indéniable(s). Toutefois, encore faut-il reconnaître... En somme...*

### Flashcard spelling traps — write each word 5x (20 min)
- [ ] apprentissage
- [ ] responsabilité
- [ ] notamment
- [ ] expériences
- [ ] dépendance
- [ ] sociaux
- [ ] privilégiés
- [ ] environnementaux
- [ ] années
- [ ] célèbre
- [ ] campagne
- [ ] apprécié

---

## Session 2: Timed Full Simulation #1 (1 hour)

### Prompts

**Tâche 1 (15 min):**
> Vous avez trouvé un emploi dans une autre ville. Écrivez un message à votre ami(e) pour lui annoncer la nouvelle et lui décrire votre futur logement.

**Tâche 2 — Frame A narration (20 min):**
> Vous avez participé à un cours de cuisine. Racontez cette expérience aux membres d'un site internet culinaire. Expliquez ce que vous avez le plus aimé.

**Tâche 3 (25 min):**
> Document 1: Le télétravail permet aux employés de mieux gérer leur temps et réduit le stress lié aux transports. Les études montrent une augmentation de la productivité.
> Document 2: Le télétravail isole les employés et rend la communication plus difficile. Les frontières entre vie privée et vie professionnelle disparaissent.

### Rules
- [ ] Set timer: 55 minutes total
- [ ] Write on paper with pen
- [ ] Use STRUCTURE files as reference (allowed this time)
- [ ] Write Tâche 1
- [ ] Write Tâche 2
- [ ] Write Tâche 3

### Self-check after (5 min)
- [ ] Every adjective agrees? (féminin/pluriel)
- [ ] Every TU verb ends in -s?
- [ ] Plural subjects have plural verbs?
- [ ] Prépositions correct? (sur, à, de)
- [ ] Spelling of known traps?
- [ ] No vous/tu mixing?

---

## Session 3: Self-correct + Speed Drills (1 hour)

### Correct simulation #1 (30 min)
- [ ] Read through all 3 texts slowly
- [ ] Circle every error found
- [ ] Rewrite only the sentences that had errors (by hand)

### Speed drill: 2 extra Tâche 2s — 10 min each (30 min)

**Prompt A — Frame E mixed (10 min):**
> Vous avez participé à une activité de bénévolat. Racontez votre expérience à vos amis et donnez votre avis sur ce type d'activité.

**Prompt B — Frame D opinion (10 min):**
> Un ami vous dit qu'il veut arrêter ses études pour lancer une entreprise. Que pensez-vous de cette décision ? Écrivez-lui un message pour donner votre opinion.

- [ ] Write Prompt A — Frame E (narration + opinion)
- [ ] Write Prompt B — Frame D (pure opinion)
- [ ] Quick self-check on both

---

# DAY 2

## Session 1: Timed Full Simulation #2 — NO REFERENCE (1 hour)

### Prompts

**Tâche 1 (15 min):**
> Vous cherchez un(e) baby-sitter pour vos enfants. Vous écrivez une annonce en décrivant les conditions (horaires, lieu, rémunération, etc.).

**Tâche 2 — Frame E mixed (20 min):**
> Vous avez assisté à un concert. Écrivez un message à vos amis pour leur raconter cette expérience et leur donner votre avis.

**Tâche 3 (25 min):**
> Document 1: Les réseaux sociaux permettent de maintenir le contact avec ses proches et de s'informer rapidement. Ils favorisent la liberté d'expression.
> Document 2: Les réseaux sociaux créent une dépendance et diffusent de fausses informations. Le cyberharcèlement touche de plus en plus de jeunes.

### Rules
- [ ] Set timer: 55 minutes total
- [ ] Write on paper with pen
- [ ] **NO looking at structure files**
- [ ] Write Tâche 1
- [ ] Write Tâche 2
- [ ] Write Tâche 3

### Self-check after (5 min)
- [ ] Every adjective agrees? (féminin/pluriel)
- [ ] Every TU verb ends in -s?
- [ ] Plural subjects have plural verbs?
- [ ] Prépositions correct? (sur, à, de)
- [ ] Spelling of known traps?
- [ ] No vous/tu mixing?

---

## Session 2: Correct + Targeted Drills (1 hour)

### Correct simulation #2 (30 min)
- [ ] Compare with structure files now
- [ ] Note any recurring errors
- [ ] Rewrite sentences with errors

### Speed drill: Tâche 1 x2 — 7 min each (15 min)

**Prompt A (7 min):**
> Votre ami vous a demandé de lui recommander un film. Écrivez-lui un message.

**Prompt B (7 min):**
> Vous avez trouvé un objet perdu dans un parc. Écrivez un message au bureau des objets trouvés.

- [ ] Write Prompt A
- [ ] Write Prompt B

### T3 skeleton from memory x3 (15 min)
- [ ] Attempt 1 — write from memory, then check against STRUCTURE-T3.md
- [ ] Attempt 2 — write from memory, then check
- [ ] Attempt 3 — should be perfect

---

## Session 3: Final Review + Confidence Building (1 hour)

### Reread best corrected essays (20 min)
- [ ] Skim through corrected versions in `/tcf-writing-practice/`
- [ ] Read for patterns to repeat, not content

### Write exam cheat sheet by hand — one page only (20 min)
- [ ] T1 informal opening
- [ ] T1 formal opening
- [ ] T1 informal closing
- [ ] T1 formal closing
- [ ] T2 opening + emotional label (Frame A)
- [ ] T2 highlight sentence
- [ ] T2 reflection sentence
- [ ] T2 closing (Frame A)
- [ ] T2 opinion opening (Frame D): *Je suis convaincu(e) que...*
- [ ] T2 opinion support (Frame D): *Mon expérience m'a appris que...*
- [ ] T2 narration-to-opinion transition (Frame E): *Personnellement, j'ai trouvé que...*
- [ ] T3 P1 skeleton
- [ ] T3 P2 skeleton
- [ ] T3 P3 skeleton
- [ ] Top 5 error checklist (accords, tu -s, prépositions, spelling)

### Final speed drill — no checking, just write (20 min)

**Tâche 2 — Frame D opinion (10 min):**
> Courrier des lecteurs : « Faut-il interdire les voitures dans les centres-villes ? » Donnez votre opinion sur ce sujet.

**Tâche 3 (10 min):**
> Doc 1: Les voitures électriques réduisent la pollution et sont moins chères à entretenir.
> Doc 2: La production de batteries pollue et les bornes de recharge sont insuffisantes.

- [ ] Write T2 — don't stop, keep moving
- [ ] Write T3 — don't stop, keep moving

---

# Final checklist — exam day morning

- [ ] Reread your handwritten cheat sheet one last time
- [ ] Remind yourself: simple and correct > complex and broken
- [ ] Remind yourself: 1-2 clean corrections on paper = normal
- [ ] Remind yourself: T1 = 15 min, T2 = 20 min, T3 = 25 min
- [ ] Bring 2 pens (backup!)
